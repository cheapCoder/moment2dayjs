...
const transform = (file, { j, report, stats }, option) => {
    ...
    try {
        ...
        (_a = root
            .find(j.ImportDeclaration, { source: { value: 'moment' } })) === null || _a === void 0 ? void 0 : _a.find(j.ImportDefaultSpecifier).forEach((path) => {
            context.importName = path.node.local.name;
        });
        // ------------------------- replace static method --------------------------------------------
        config_1.staticTransform.forEach((conf) => {
            root
                .find(j.CallExpression, (node) => node.callee.type === 'MemberExpression' &&
        ...
        config_1.methodTransform.forEach((conf) => {
            root
                .find(j.CallExpression, (node) => { var _a, _b; return conf.name.test((_b = (_a = node.callee) === null || _a === void 0 ? void 0 : _a.property) === null || _b === void 0 ? void 0 : _b.name); })
                .replaceWith((path) => {
                // if (conf.transform) {
        ...
        root.find(j.ImportDeclaration, { source: { value: 'moment' } }).replaceWith((path) => {
            ...
            path.node.specifiers = (_a = path.node.specifiers) === null || _a === void 0 ? void 0 : _a.map((s) => {
        ...
        root
            ...
            .replaceWith((path) => {
            var _a;
            if ((_a = path.node.declarations) === null || _a === void 0 ? void 0 : _a[0]['init'].arguments[0]) {
        ...
        const globalImport = plugins.map((name) => `import ${name} from 'dayjs/plugin/${name}';`).join('\n') +
            '\n\n' +
